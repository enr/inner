...
A **one-shot** run starts Claude Code in non-interactive mode, executes a single task, and exits. It is suitable for CI pipelines, scripted automation, and batch processing.

The `one-shot` built-in profile configures:
...
```bash
inner run -p one-shot -w ~/projects/myapp --prompt "add type annotations to all Python functions"
```

**What happens:**
1. `inner` builds a sandbox using the `one-shot` profile
...
```bash
inner run -p one-shot -w ~/projects/myapp --timeout 300 \
...
```bash
inner run -p one-shot --network -w ~/projects/myapp \
...
inner run -p one-shot -w "$(pwd)" --timeout 600 \
...
```bash
inner run -p one-shot -w ~/myapp -- --model claude-opus-4-5 --print "explain the architecture"
```
...
`inner` does not natively run Docker images, but the `agent-containers` profile grants Claude Code access to a **Podman rootless** socket so the agent can build and run containers inside the sandbox.

The `agent-containers` profile sets:
...
```bash
inner run -p agent-containers -w ~/projects/myapp
```

Inside this sandbox, Claude Code can:

```bash
# These all work inside the agent-containers sandbox:
...
```bash
inner verify -p agent-containers
```
...
inner run -p agent-interactive -w ~/myapp -m ~/datasets:/data:ro

# Read-write shared build artifacts
inner run -p agent-interactive -w ~/myapp -m /tmp/artifacts:/artifacts:rw
```
...
```bash
inner run -p agent-interactive -w ~/myapp --dry-run
```
